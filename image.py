...
def make_image(result: match):
    ...
    scoreDetail = result.scoreDetail.replace(" ","")
    # 試合日を画像の中央上部に描画
    make_text(draw,400,20,f"J1リーグ {result.category[2:]} {result.date}",)
    make_text(draw,155,90,result.hometeam)
    make_text(draw,600,90,result.awayteam)
    make_text(draw,345,160,result.venue)
    make_text(draw,160,280,"１")
    make_text(draw,390,260,scoreDetail[1])
    make_text(draw,620,280,scoreDetail[2])
    
    # 画像を保存するファイル名を設定（カテゴリーに基づく命名）
    output_file = f"soccer_{result.category}.png"
    image.save(output_file)  # 画像を保存
    print(f"画像を保存しました: {output_file}")  # 保存完了メッセージを表示

def make_text(draw,x,y,text):
